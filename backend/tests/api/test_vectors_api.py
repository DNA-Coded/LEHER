...
def test_valid_vector_slice_request_arrow_round_trip(mock_fabric, monkeypatch):
    from apps.api.leher.paths import REPO_ROOT
    monkeypatch.setattr("apps.api.leher.routers.vectors.REPO_ROOT", mock_fabric.parent)
    
    # Clear cache to prevent cross-test contamination
    from apps.api.leher.routers import vectors
    vectors._slice_cache.clear()
    ...
    assert "u_velocity" in response.headers["X-Variables"]
    assert "v_velocity" in response.headers["X-Variables"]
    
    reader = ipc.RecordBatchStreamReader(io.BytesIO(response.content))
    table = reader.read_all()
    df = table.to_pandas()
    
    assert set(df.columns).issuperset({"latitude", "longitude", "u_velocity", "v_velocity", "current_speed"})
